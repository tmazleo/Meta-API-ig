...
type InstagramMedia struct {
	...
	Timestamp        MetaTime `json:"timestamp" bigquery:"-"`                             // Data/hora da publicação
	...
	Views               int32     `json:"-" bigquery:"views"`
	Replies             int32     `json:"-" bigquery:"replies"`
	Shares            	int32 	`json:"-" bigquery:"shares"`
	TotalInteractions 	int32 	`json:"-" bigquery:"total_interactions"`                      // Views (para vídeos/reels)
...
type InsightData struct {
	Name   string         `json:"name"`
	Values []InsightValue `json:"values"` // Note que é um slice/array
}
